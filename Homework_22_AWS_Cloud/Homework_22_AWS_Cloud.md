...
+ *Написать скрипты `userdata` для каждой `EC2` которые будут вытягивать в `S3` нужный нам html файл и ложить его в директорию с которой работает сервер `Apache`, который нужно установить. Важно, `userdata` отрабатывает только при первом запуске*
